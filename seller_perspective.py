...
# NOTE: st.set_page_config removed — parent app (vob_minimal.py) owns it.

# -----------------------
#  CUSTOM CSS - SELLER THEME + NEW MOMENT FEATURES + EXPIRY SPIKE + OI/PCR + ATM BIAS
# -----------------------
def _inject_seller_perspective_css():
    st.markdown(r"""
<style>
...
def get_nifty_spot_price():
    ...
    Prefers data already cached in st.session_state by vob_minimal.py to avoid
    duplicate Dhan API calls. Falls back to a direct Dhan request only if
    session state has nothing useful.
    """
    # Reuse cached underlying from minimal app if present
    try:
        cached = st.session_state.get('_last_underlying')
        if cached:
            return float(cached)
        opt = st.session_state.get('_cached_option_data')
        if isinstance(opt, dict):
            und = opt.get('underlying')
            if und:
                return float(und)
    except Exception:
        pass
...
@st.cache_data(ttl=60)
def get_expiry_list():
    # Reuse cached expiry list from minimal app if available
    try:
        opt = st.session_state.get('_cached_option_data')
        if isinstance(opt, dict):
            exps = opt.get('expiry_dates')
            if exps:
                return list(exps)
    except Exception:
        pass
...
def render_nifty_option_screener():
    ...
    **Data:** Dhan API required
    ...
    st.markdown("---")
    st.markdown(f"**🔄 Last update:** Parent app handles refresh every {AUTO_REFRESH_SEC} seconds")
    # NOTE: st_autorefresh removed — vob_minimal.py owns the refresh cycle.

# -----------------------
#  TAB ENTRY POINT (used by vob_minimal.py)
# -----------------------
def render_seller_perspective_tab():
    """Entry point used by vob_minimal.py to render the Seller's Perspective tab."""
    try:
        _inject_seller_perspective_css()
    except Exception:
        pass
